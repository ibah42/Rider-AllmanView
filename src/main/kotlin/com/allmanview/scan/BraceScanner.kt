...
class BraceScanner(
    ...
    /** States a newline ends, whatever the text says: the reset that bounds a desync to a line. */
    private fun isSingleLine(state: LexerState): Boolean {
        return state == LexerState.LINE_COMMENT ||
    ...
    private fun finishLine(lineEndOffset: Int) {
        ...
        if (isSingleLine(lexerState)) {
            // do not carry a desync any further
            lexerState = LexerState.CODE
            interpolationDollars = 0
        }

        // An interpolation hole opened inside a single-line literal cannot outlive the line
        // either, and the check above cannot see it: inside a hole the lexer is in CODE, which
        // looks perfectly healthy. Half-typed `var s = $"v: {x` would otherwise carry the hole
        // to the end of the file -- every block after it left unclosed -- which is exactly what
        // the reset exists to prevent. The outermost frame is the one that decides: if the
        // literal the nest started in was single-line, the whole nest belongs to this line.
        //
        // A verbatim or raw literal is left alone on purpose. Its hole may legitimately span
        // lines, and nothing at a newline can tell that apart from an unclosed one.
        if (interpolationStack.isNotEmpty() && isSingleLine(interpolationStack.first().lexerState)) {

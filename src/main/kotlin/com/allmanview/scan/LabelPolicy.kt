...
object LabelPolicy {
     ...
     * own kind and has reached its own, usually shorter, length ([nestedLabelAlways],
     * [namesNestedBlock]); or it is numbered and the number is being repeated down here, since
     * `[3]` alone says nothing ([numbersEndOfBlock]). [showLabel] -- the per-kind "label this
     * kind at all" switch -- sits above all three.
     *
     * Nesting used to name a block at any length at all, with no minimum of its own -- unlike
     * numbering, which has always had one. Both now answer to a length of their own, shorter
     * than a plain block's rather than zero: "always at the top of the block, never mind how
     * short; not always at the bottom" holds for both markers alike.
     *
    ...
    fun needsLabel(
        ...
        if (namesNestedBlock(accent, nestedLabelAlways, nestedLabelMinLines)) {
            return true
        }
    ...
    /**
     * Whether a nested block is named for being nested, rather than for its own length: the
     * switch is on, the block is nested, and it has reached [minLines] -- the same length the
     * `nest` word itself starts repeating at on the closing brace (see [marksAsNested]), so a
     * one-line nested block no longer earns a full label just for being nested, the same way a
     * one-line numbered block no longer earns one just for carrying a `[1]`.
     */
    fun namesNestedBlock(accent: BraceAccent, nestedLabelAlways: Boolean, minLines: Int): Boolean {
        if (!nestedLabelAlways) {
            return false
        }
    ...
    /**
     * Whether the `nest` word is drawn at [accent]'s position: on the declaration line it
     * always is, since the block it names sits right next to it; on the closing brace it has
     * to earn its place the same way the sibling ordinal's `[N]` does in [numbersEndOfBlock],
     * since by then the declaration may be long out of view.
     */
    fun marksAsNested(accent: BraceAccent, nestedMarkerEnabled: Boolean, endOfBlockMinLines: Int): Boolean {
        if (!nestedMarkerEnabled) {
            return false
        }
        if (!isNestedBlock(accent)) {
            return false
        ...
        return accent.spannedLines >= endOfBlockMinLines
    }

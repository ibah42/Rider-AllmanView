...
class BraceScannerTest {
    ...
    fun `constructor with a base call initializer on its own line keeps its name`() {
        val src = "public Depot()\n    : base(Interface)\n{\n}"
        assertEquals(BlockKind.FUNCTION, accents(src).first().kind)
        assertEquals("Depot", accentName(src, accents(src).first()))
        assertEquals("ctor", accents(src).first().keyword)
    }
    ...
    fun `a constructor with several default parameter values is still recognised`() {
        ...
        assertEquals(BlockKind.FUNCTION, accents(src).first().kind)
        assertEquals("ctor", accents(src).first().keyword)
        assertEquals("Parcel", accentName(src, accents(src).first()))
    }
    ...
    fun `name length allows slicing it out`() {
        val src = "public class CrateShelf {\n}"
    ...
    fun `namespace is its own kind, neither TYPE nor FUNCTION`() {
        val namespaceAccents = accents("namespace Com.Ivv.Core {\n}")
    ...
    fun `generic method with a multi-line where clause and an Allman brace is still a function`() {
        ...
        val src = "public async UniTask<Parcel<TPayload>> Deliver<TPayload>(\n" +
            "    Crate crate,\n" +
            "    Whistle whistle\n" +
            ")\n" +
            ...
            "}"
        assertEquals(BlockKind.FUNCTION, accents(src).first().kind)
        assertEquals("Deliver", accentName(src, accents(src).first()))
    }
    ...
    // --- the C# string zoo ------------------------------------------------------------------
    //
    // Measured by how the braces PAIR, not by which blocks open. A literal that swallows a real
    // brace leaves the opening list untouched and mis-pairs everything after it, so a test that
    // only counts openings passes while the file is being read wrong.

    /** Every closing brace as `keyword@line`, which is what a swallowed brace disturbs. */
    private fun closingsIn(src: String, flavor: Flavor = Flavor.CSHARP): List<String> {
        ...
        return BraceScanner(src, flavor, ScanOptions()).scan().accents
            .filter { !it.isOpening }
            .sortedBy { it.offset }
            .map { it.keyword + "@" + lineOf(it.offset) }
    }

    /** The payload goes on line 5 of a file whose method closes on 6 and whose class closes on 7. */
    private fun assertPayloadIsInert(title: String, payload: String) {
        val src = "class C\n{\n    void M()\n    {\n        " + payload + "\n    }\n}\n"
        assertEquals(
            title + " -> [fun@6, class@7]",
            title + " -> " + closingsIn(src),
        )
    }

    @Test
    fun `every single-line C sharp literal keeps the braces around it intact`() {
        // Braces are escaped the way each form requires, which differs between the forms and is
        // half the reason the dialect exists.
        val cases = listOf(
            "plain, hanging brace" to "var s = \"if (x) {\";",
            "plain, a whole class" to "var s = \"class Z { void N() { } }\";",
            "plain, looks like a comment" to "var s = \"// class Z { }\";",
            "plain, looks like a block comment" to "var s = \"/* class Z { } */\";",
            "plain, apostrophe inside" to "var s = \"it's a brace: {\";",
            "plain, escaped quote" to "var s = \"quote \\\" then brace {\";",
            "plain, utf8 suffix" to "var s = \"class Z {\"u8;",
            "verbatim, brace" to "var s = @\"a { b\";",
            "verbatim, doubled quote" to "var s = @\"say \"\"hi\"\" and {\";",
            "verbatim, backslash" to "var s = @\"C:\\path\\to\\{x}\";",
            "interpolated, hole" to "var s = $\"class {name} {{\";",
            "interpolated, escaped braces" to "var s = $\"{{ not a hole }}\";",
            "interpolated, triple brace" to "var s = $\"{{{x}}}\";",
            "interpolated, format specifier" to "var s = $\"0x{value:X8} {{\";",
            "interpolated, alignment" to "var s = $\"{value,10} {{\";",
            "interpolated, hole holding a string" to "var s = $\"{string.Format(\"{0}\", x)} {{\";",
            "interpolated, hole holding a lambda" to "var s = $\"{Map(xs, x => { return x; })} {{\";",
            "interpolated, nested interpolation" to "var s = $\"{ $\"{inner}\" }\";",
            "verbatim interpolated" to "var s = $@\"a {x} b {{ c\";",
            "interpolated verbatim, other order" to "var s = @$\"a {x} b {{ c\";",
            "empty literal then a brace" to "var a = \"\"; var b = \"{\";",
        )
        for ((title, payload) in cases) {
            assertPayloadIsInert(title, payload)
        }
    }

    @Test
    fun `a multi-line verbatim string can hold a whole class`() {
        val src = "class C\n{\n    void M()\n    {\n        var s = @\"\nclass Z\n{\n    void N()\n    {\n    }\n}\n\";\n    }\n}\n"
        assertEquals(listOf("fun@13", "class@14"), closingsIn(src))
    }

    @Test
    fun `a raw string literal can hold a whole class`() {
        val src = "class C\n{\n    void M()\n    {\n        var s = \"\"\"\nclass Z\n{\n}\n\"\"\";\n    }\n}\n"
        assertEquals(listOf("fun@10", "class@11"), closingsIn(src))
    }

    @Test
    fun `a longer fence lets the content hold a shorter one`() {
        val src = "class C\n{\n    void M()\n    {\n        var s = \"\"\"\"\nhe said \"\"\" and left a brace {\n\"\"\"\";\n    }\n}\n"
        assertEquals(listOf("fun@8", "class@9"), closingsIn(src))
    }

    @Test
    fun `a raw interpolated string opens a hole on one brace`() {
        val src = "class C\n{\n    void M()\n    {\n        var s = $\"\"\"\nclass {name}\n\"\"\";\n    }\n}\n"
        assertEquals(listOf("fun@8", "class@9"), closingsIn(src))
    }

    @Test
    fun `two dollars make a single brace literal and a doubled one a hole`() {
        // The form that exists so a template can hold C# bodies with no escaping at all.
        val src = "class C\n{\n    void M()\n    {\n        var s = $$\"\"\"\nclass {{name}}\n{\n    void Work() { }\n}\n\"\"\";\n    }\n}\n"
        assertEquals(listOf("fun@11", "class@12"), closingsIn(src))
    }

    @Test
    fun `three dollars need three braces to open a hole`() {
        val src = "class C\n{\n    void M()\n    {\n        var s = $$$\"\"\"\n{{ stays literal }} and {{{name}}} is a hole\n\"\"\";\n    }\n}\n"
        assertEquals(listOf("fun@8", "class@9"), closingsIn(src))
    }

    @Test
    fun `a brace inside an interpolation hole opens no block`() {
        // The hole is real code, but a phantom brace line inside a string literal would be
        // nonsense, so braces there are counted and never reported.
        val src = "class C\n{\n    void M()\n    {\n        var s = $\"{Map(xs, x => { return x; })}\";\n    }\n}\n"
        assertEquals(listOf("class", "fun"), openings(src).map { it.keyword })
        assertEquals(listOf("fun@6", "class@7"), closingsIn(src))
    }

    @Test
    fun `a half-typed interpolation is bounded to its own line`() {
        // `$"v: {x` with the hole still open. Inside a hole the lexer sits in CODE, so the
        // newline reset cannot see it by state alone -- and without the unwind this ran to the
        // end of the file, leaving every block after it unclosed.
        assertPayloadIsInert("unclosed hole", "var s = $\"v: {x;")
        assertPayloadIsInert("lone open brace", "var s = $\"{\";")
        assertPayloadIsInert("unterminated plain string", "var s = \"oops;")
    }

    @Test
    fun `an unterminated verbatim string runs to the end of the file, on purpose`() {
        // Not a bug to fix: a verbatim literal must survive a newline or a real multi-line
        // string breaks, and at the newline there is nothing to tell the two apart. Asserted so
        // that changing it is a decision rather than an accident.
        val src = "class C\n{\n    void M()\n    {\n        var s = @\"oops;\n    }\n}\nclass D\n{\n}\n"
        assertEquals(emptyList<String>(), closingsIn(src))
    }
